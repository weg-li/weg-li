class Scheduled::UsageReminderJob < ApplicationJob
  def perform
    ...
    not_using = User.active.left_joins(:notices).where('notices.id IS NULL').where("date_part('dow', users.updated_at) = ?", Date.today.wday).where('users.updated_at < ?', 2.month.ago)
    not_using.each do |user|
      if user.updated_at < 3.month.ago
