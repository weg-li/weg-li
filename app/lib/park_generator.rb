...
class ParkGenerator
  def generate(name)
    pdf = Prawn::Document.new do |document|
      ...
      Rails
        .root
        .join("app/assets/images/stadtreinigung.png")
        .open do |file|
          document.image(
            file,
            fit: [100, 100],
      ...
      text = "
      <i>
      Sie haben Ihr Fahrzeug verkehrsbehindernd abgestellt.

      <color rgb='ff0000'>Das bedeutet: Der Einsatz von Stadtreinigung und Feuerwehr wird erheblich eingeschränkt.</color>
      </i>
      "
      document.text_box(
        text,
      ...
      Rails
        .root
        .join("app/assets/images/feuerwehr.jpg")
        .open do |file|
          document.image(
            file,
