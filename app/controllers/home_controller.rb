class HomeController < ApplicationController
  ...
  def leaderboard
    @weekly_leaders = Notice.since(Time.zone.now.beginning_of_week).shared.group(:user_id).limit(10).count
    @weekly_leaders.transform_keys! { |user_id| User.find(user_id) }
    @monthly_leaders = Notice.since(Time.zone.now.beginning_of_month).shared.group(:user_id).limit(10).count
    @monthly_leaders.transform_keys! { |user_id| User.find(user_id) }
    @yearly_leaders = Notice.since(Time.zone.now.beginning_of_year).shared.group(:user_id).limit(10).count
    @yearly_leaders.transform_keys! { |user_id| User.find(user_id) }
    @total_leaders = Notice.shared.group(:user_id).limit(10).count
